...
class LabelSmoothing(torch.nn.Module):
    ...
    def forward(self, x, target):
        ...
        if torch.max(target) >= self.size or torch.min(target) < 0:
            raise ValueError(f"Valores de target fuera del rango permitido: {torch.min(target)} a {torch.max(target)}, esperado en rango [0, {self.size - 1}]")
        
        #print(f"x shape: {x.shape}, target shape: {target.shape}, max target: {torch.max(target)}, min target: {torch.min(target)}")
